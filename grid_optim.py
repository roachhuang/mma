...
import mykbar as kbars

'''
#########################################
# MACD指標 Moving Average Convergence Divergence
# the momentum and trend of an asset's price
###########################################
# 使用快慢線交叉當作買賣訊號
def createSignalMACD(close, periodFast, periodSlow, periodSignal):
        ...
        for periodSlow in rangeSlow:
            for periodSignal in rangeSignal:
                ...
                if periodFast >= periodSlow:
                    continue
                # 製作買賣訊號
                BuySignal = createSignalMACD(
                    closePrice, periodFast, periodSlow, periodSignal
                )
                # 對訊號進行回測
...
###########################################
# 使用KD交叉當作買賣訊號
def createSignalKD(high, low, close, fastk=5, slowk=3, slowd=3):
    ...
    for fastk in range_fastk:
        for slowk in range_slowk:
            for slowd in range_slowd:
                ...
                # 製作買賣訊號
                BuySignal = createSignalKD(
                    highPrice, lowPrice, closePrice, fastk, slowk, slowd
                )
                # 對訊號進行回測
                retStrategy, ret_series = backtesttool.backtest_signal(
                    openPrice, BuySignal
...
def createSignalRSI(close, timeperiod=14, longTH=70, shortTH=30):
        ...
        for longTH in range_longTH:
            for shortTH in range_shortTH:
                ...
                if longTH <= shortTH:
                    continue
                # 製作買賣訊號
                BuySignal = createSignalRSI(closePrice, period, longTH, shortTH)
                # 對訊號進行回測
                retStrategy, ret_series = backtesttool.backtest_signal(
                    openPrice, BuySignal
                )
...
def createSignalBBAND(close, timeperiod=20, SmallStdDev=1.0, LargeStdDev=2.0):
    upperband_Small, middleband_Small, lowerband_Small = talib.BBANDS(
        close,
    ...
    upperband_Large, middleband_Large, lowerband_Large = talib.BBANDS(
        close,
        timeperiod=timeperiod,
        nbdevup=LargeStdDev,
        nbdevdn=LargeStdDev,
    ...
    # 允許放空的訊號寫法
    if ENABLESHORT:
        BuySignal = ((close >= upperband_Small) & (close <= upperband_Large)).astype(
            int
        )
        ShortSignal = ((close >= lowerband_Large) & (close <= lowerband_Small)).astype(
            int
        )
        return BuySignal - ShortSignal
    # 不允許放空的訊號寫法，兩個差在允許放空的部分多了ShortSignal
...
def createSignalPriceChannel(df, period):
    high = df["High"]
    low = df["Low"]
    # 創新高買進訊號
...
# 找出價格通道買賣訊號的最佳化參數
def OptimizePriceChannel(df, range_period):  # =numpy.arange(2,100,1,dtype=int)
    openPrice = df["Open"]
    closePrice = df["Close"]
    bestret = 0
    bestret_series = []
    ...
    for period in range_period:
        ...
        # 對訊號進行回測
        retStrategy, ret_series = backtesttool.backtest_signal(openPrice, BuySignal)
        # 如果結果比之前更好,就記錄下來
        if bestret < retStrategy:
            bestret = retStrategy
            bestret_series = ret_series
...
def createGridSignal(
    ...
    close = df["Close"]
    #calculates a bias ratio for each closing price in the close series
    Bias = close / close.rolling(window=BiasPeriod).mean()
    # Backfill replaces missing values with the value from the previous period
    # Bias = Bias.fillna(method="bfill")
    Bias = Bias.bfill()
    
    positiondiff = UpperLimitPosition - LowerLimitPosition
    biasdiff = BiasUpperLimit - BiasLowerLimit
    # The position increases or decreases as the bias ratio goes above or below the defined limits
...
if __name__ == "__main__":
    ...
    symbol = "0052"
    contract = kbars.getContract(api, name=symbol, the_type="stock")
    start = kbars.sub_N_Days(days=15 * 365)
    ...
    # 從資料庫讀取小型台指歷史資料
    df_MXFR1 = kbars.readFromDB('kbars',  symbol)
    ...
    if target == "KD":
        ...
        bestret, bestret_series, parameters = OptimizeKD(
            df_MXFR1, range_fastk, range_slowk, range_slowd
        )
    ...
    if target == "RSI":
        ...
        bestret, bestret_series, parameters = OptimizeRSI(
            df_MXFR1, range_period, range_longTH, range_shortTH
        )
    ...
    if target == "BBAND":
        ...
        bestret, bestret_series, parameters = OptimizeBBAND(
            df_MXFR1, range_period, range_SmallStdDev, range_LargeStdDev
        )
    ...
    if target == "Grid":
        ...
        TW_hist = tw.history(period="5y")   # max 5 years
        ...
        print(f'BEST bias_upper, upperPos, biasLower, lowerPos, period: {parameters}')
        ...
        plt.show()
        plt.plot(numpy.log10(TW_close / TW_close.iloc[0]), color="blue")
        plt.title("TW Profit(log)")
        plt.show()
        plt.plot(numpy.log10(US_close / US_close.iloc[0]), color="red")
