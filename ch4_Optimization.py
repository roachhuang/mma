...
symbol = "0052.TW"
# 畫均線
df_MXFR1 = gd.getData(symbol=symbol, interval="1h", years=2/12) # max 60 days for interval < 1d
# df_MXFR1 = kbars.readFromDB("1m", symbol)
# df_FXFR1=kbars.readFromDB('kbars', 'FXFR1')
# df_MXFR1=kbars.readFromDB('kbars', 'MXFR1')
# df_FXFR1=kbars.resampleKbars(df_FXFR1,'1h')

# df_MXFR1 = kbars.resampleKbars(df_MXFR1, "1h")
close_price = df_MXFR1["Close"]
ma100 = ta.ema(close_price, 100)
...
plt.legend(["ma50:red", "ma100:blue"])
plt.show()
'''
...
# from start date of kbar till end date of kbar collection.
######################################
...
# total profit in openprice range
def backtest_signal(openprice, signal, tradecost=G_tradecost, sizing=1.0):
    ...
    if not position.empty:
        ...
        print("Series is empty. Cannot assign value.")
    # list() io比series快
    ...
    for i in range(0, position.size - 1, 1):
        try:
            ...
            try:
                ...
                pass
            # if no buy/sell on the day, ret is 1
...
retStrategy, ret_series = backtest_signal(
    open_price, BuySignal, tradecost=G_tradecost, sizing=1.0
)
print(f"均線策略報酬率: {round(retStrategy*100,2)}%")
...
# 計算MDD，最大虧損, Lower Maximum Drawdown values indicate lower risk
...
retStrategyHold, ret_seriesHold = backtest_signal(
    open_price, HoldSignal, tradecost=G_tradecost, sizing=1.0
)
MDD = calculateMDD(ret_seriesHold)
print(f"買進持有報酬率: {round(retStrategyHold*100,2)}%")
print("買進持有MDD:", MDD)
...
def optimizeMA(
    ...
    def createSignal(close, periodShort, periodLong):
        maShort = ta.ema(close, periodShort)
        maLong = ta.ema(close, periodLong)
    ...
    for periodLong in period_range_Long:
        for periodShort in period_range_Short:
            ...
            # 對訊號進行回測
            retStrategy, ret_series = backtest_signal(
                openPrice, BuySignal, tradecost=G_tradecost, sizing=1.0
            )
            # 如果結果比之前更好,就記錄下來
...
ret, ret_series, bestperiod = optimizeMA(
    df_MXFR1, numpy.arange(2, 100, 1, dtype=int), numpy.arange(2, 100, 1, dtype=int)
)

MDD = calculateMDD(ret_series)
print(f"最佳化的報酬率: {round(ret*100,2)}%")
print("最佳化的MDD:", MDD)
...
date = kbars.sub_N_Days(days=120, date=df_MXFR1.index[-1])
df_trainset = df_MXFR1[df_MXFR1.index <= date]
df_testset = df_MXFR1[df_MXFR1.index > date]
# 用訓練集跑最佳化
ret_train, ret_series_train, bestperiod_train = optimizeMA(
    df_trainset, numpy.arange(2, 100, 1, dtype=int), numpy.arange(2, 100, 1, dtype=int)
)

# 用測試集測試最近四個月的績效
ma_short = ta.EMA(df_testset["Close"], bestperiod[1])
ma_long = ta.EMA(df_testset["Close"], bestperiod[0])
...
retStrategy_Outsample, ret_series_Outsample = backtest_signal(
    df_testset["Open"], buy_signal, tradecost=G_tradecost, sizing=1.0
)
# 前面沒有分訓練集測試集的三個月內報酬
