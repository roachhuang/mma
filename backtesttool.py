...
# index of start kbar, and index of end kbar. return profit in this period
...
def backtest_signal(openprice, signal, tradecost=G_tradecost, sizing=1.0):
    ...
    ####################################
    # 把買賣訊號往後移一天 (move down one row; date index unchanged),因為今天收盤的訊號下一天開盤才會買賣
    ####################################
    # array of 0 or 1, signal of buy or sell
    ...
    # list() io比series快. length of below 2 lists are the same
    ...
    for periodLong in period_range_Long:
        for periodShort in period_range_Short:
            if periodLong <= periodShort:
                continue  # for granted, long must be > than short
            # 製作買賣訊號
...
# input: array of daily profit, output: profit trend
...
# input: array of daily profit, output: Max Draw Down
